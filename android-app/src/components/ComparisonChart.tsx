...
type HoverLayerHandle = {
    updateHover: (index: number) => void;
    clearHover: () => void;
};

const HoverLayer = React.memo(React.forwardRef<HoverLayerHandle, any>(({
    seriesList,
    labels,
    getX,
    getY,
    ...
    if (hoverIndex === null || seriesList.length === 0) return null;
    const basePoint = seriesList[0].points[hoverIndex];
    if (!basePoint) return null;

    return (
        <>
            <View style={[styles.tooltip, {
                left: Math.max(10, Math.min(chartWidth - 190, getX(basePoint.time) - 100)),
                backgroundColor: colors.bgMain + 'F2'
            }]}>
                <Text style={styles.tooltipTime}>
                    {new Date(basePoint.time).toLocaleDateString()} {new Date(basePoint.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </Text>
            ...
            <Svg width={chartWidth} height={height} style={styles.svgOverlay} pointerEvents="none">
                <Line
                    x1={getX(basePoint.time)} y1="0"
                    x2={getX(basePoint.time)} y2={height}
...
export const ComparisonChart = ({ data, symbols, labels = new Map(), interval, height = 300 }: ComparisonChartProps) => {
    ...
    const hoverLayerRef = useRef<HoverLayerHandle>(null);
    ...
    const handleTouch = (evt: any) => {
        ...
        if (closest !== null) hoverLayerRef.current?.updateHover(closest);
    };
    ...
    return (
        <View style={[styles.container, { height }]}>
            <Pressable
                ...
                onPressOut={() => hoverLayerRef.current?.clearHover()}
